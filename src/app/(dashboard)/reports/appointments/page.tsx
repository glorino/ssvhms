...
import { ArrowLeft, Download, Calendar, TrendingUp, CheckCircle, XCircle } from "lucide-react"
...
const summaryCards = [
  { label: "Total Appointments", value: "248", icon: Calendar, color: accent },
  { label: "Completed", value: "195", icon: CheckCircle, color: "#059669" },
  { label: "No-Show", value: "18", icon: XCircle, color: "#dc2626" },
  { label: "Completion Rate", value: "78.6%", icon: TrendingUp, color: "#6366f1" },
]

const appointmentData = [
  { date: "2026-08-07", scheduled: 42, completed: 35, cancelled: 3, noShow: 4, completionRate: "83.3%" },
  { date: "2026-08-06", scheduled: 38, completed: 30, cancelled: 4, noShow: 4, completionRate: "78.9%" },
  { date: "2026-08-05", scheduled: 45, completed: 38, cancelled: 2, noShow: 5, completionRate: "84.4%" },
  { date: "2026-08-04", scheduled: 35, completed: 27, cancelled: 3, noShow: 5, completionRate: "77.1%" },
  { date: "2026-08-03", scheduled: 40, completed: 32, cancelled: 4, noShow: 4, completionRate: "80.0%" },
]
    ...
    <div style={{ padding: "24px", minHeight: "100vh", background: "#f8fafc" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          ...
          <div>
            <h1 style={{ fontSize: "24px", fontWeight: 700, color: "#0f172a", margin: 0 }}>Appointment Report</h1>
            <p style={{ fontSize: "14px", color: "#64748b", margin: "4px 0 0" }}>Appointment statistics and completion rates</p>
          </div>
        </div>
      ...
      <div style={cardStyle}>
        <h2 style={{ fontSize: "16px", fontWeight: 600, color: "#0f172a", margin: "0 0 20px" }}>Daily Appointment Summary</h2>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
            <thead>
              <tr style={{ borderBottom: "2px solid #f1f5f9" }}>
                {["Date", "Scheduled", "Completed", "Cancelled", "No-Show", "Completion Rate"].map((h) => (
            ...
            <tbody>
              {appointmentData.map((row) => (
                <tr key={row.date} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  ...
                  <td style={{ padding: "12px 16px", color: "#475569" }}>{row.scheduled}</td>
                  <td style={{ padding: "12px 16px", fontWeight: 600, color: "#059669" }}>{row.completed}</td>
                  <td style={{ padding: "12px 16px", color: "#d97706" }}>{row.cancelled}</td>
                  <td style={{ padding: "12px 16px", color: "#dc2626" }}>{row.noShow}</td>
                  <td style={{ padding: "12px 16px" }}>
                    <span style={{ padding: "3px 10px", borderRadius: "12px", fontSize: "11px", fontWeight: 600, background: parseFloat(row.completionRate) >= 80 ? "#ecfdf5" : "#fef3c7", color: parseFloat(row.completionRate) >= 80 ? "#059669" : "#d97706" }}>
                      {row.completionRate}
                    </span>
                  </td>
                </tr>
